...
use data::volatility::volsurface::DivAssumptions::NoCashDivs;
...
pub trait VolSurface: esd::Serialize + TypeId + Send + Sync + Debug {
    ...
    fn forward_variances(
        ...
        if let Some(forward) = self.forward() {
            ...
            // smile correction.
            let mut from_variances = vec![f64::NAN; n];
...
where
    T: VolSmile<f64> + Clone + Debug + sd::Serialize,
{
...
impl VolByProbabilityFlatSmile {
    ...
    pub fn from_serial(de: &mut esd::Deserializer) -> Result<RcVolSurface, esd::Error> {
        ...
        match VolByProbability::new(input) {
            ...
            Err(e) => Err(esd::Error::custom(e.to_string())),
        }
    }
}
...
/// type id and deserializer.
#[derive(Debug, Serialize)]
pub struct VolByProbabilityCubicSplineSmile(VolByProbability<CubicSplineSmile<f64>>);
...
impl VolByProbabilityCubicSplineSmile {
    pub fn new(
        smiles: &[(DateDayFraction, CubicSplineSmile<f64>)],
        calendar: RcCalendar,
        base_date: DateDayFraction,
        forward: Linear<Date>,
        fixed_divs_after: Linear<Date>,
    ...
    // extra fields.
    pub fn from_serial(de: &mut esd::Deserializer) -> Result<RcVolSurface, esd::Error> {
        let input = VolByProbabilityInput::<CubicSplineSmile<f64>>::deserialize(de)?;
        match VolByProbability::new(input) {
            Ok(surface) => Ok(Qrc::new(Arc::new(surface))),
            Err(e) => Err(esd::Error::custom(e.to_string())),
        }
...
impl SsviPhi {
    pub fn new_power_law(a: f64, b: f64) -> SsviPhi {
        SsviPhi::PowerLaw(a, b)
    }

    pub fn new_heston(lambda: f64) -> SsviPhi {
        SsviPhi::Heston(lambda)
    }

    pub fn theta(&self, x: f64) -> f64 {
        match self {
            SsviPhi::PowerLaw(a, b) => a * x.powf(*b),
            SsviPhi::Heston(lambda) => {
                (1.0 / (lambda * x)) * (1.0 - (1.0 - f64::exp(-lambda * x)) / (lambda * x))
            }
        }
    }
}

impl TypeId for SsviPhi {
    fn get_type_id(&self) -> &'static str {
        "SsviPhi"
    }
}
...
impl TypeId for SSVI {
    fn get_type_id(&self) -> &'static str {
        "SSVI"
    }
}

impl VolSurface for SSVI {
    fn volatilities(
        &self,
        date_time: DateDayFraction,
        strikes: &[f64],
        ...
        let f = self.forward.interpolate(date_time.date())?;
        let xs: Vec<f64> = strikes.into_iter().map(|k| f64::ln(k / f)).collect();
        let atm = self.atm.interpolate(date_time.date())?;
        let n = strikes.len();
        for i in 0..n {
            let phi = self.phi.theta(atm);
            let w = (atm / 2.0)
                * (1.0
                    + self.rho * phi * xs[i]
                    + f64::sqrt(
                        f64::powi(phi * xs[i] + self.rho, 2) + (1.0 - f64::powi(self.rho, 2)),
                    ));
            volatilities[i] = f64::sqrt(w / date_time.day_fraction());
        }
    ...
    pub fn sample_vol_surface(base: DateDayFraction) -> VolByProbabilityCubicSplineSmile {
        ...
        smiles.push((
            ...
            CubicSplineSmile::new(
                &points,
                fwd.interpolate(ttm.date()).unwrap(),
                ttm.day_fraction(),
            )
            .unwrap(),
        ));
