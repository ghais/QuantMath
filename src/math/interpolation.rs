...
pub fn flat_interpolate_extrapolate<T: Interpolable<T> + Copy>(
    x: T,
    points: &[(T, f64)],
    left: Extrap,
    right: Extrap,
...
/// Non-flyweight linear right continuous piecewise constant interpolation. In this interpolator,
/// the data is kept internally, and passed into the constructor.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LinearRightContinuousPiecewiseConstant<T>
where
    T: Interpolable<T>,
{
    left: Extrap,
...
impl<T: Interpolable<T> + Copy> Interpolate<T> for LinearRightContinuousPiecewiseConstant<T> {
    fn interpolate(&self, x: T) -> Result<f64, qm::Error> {
        flat_interpolate_extrapolate(x, &self.points, self.left, self.right)
    }
}

impl<T: Interpolable<T> + Copy> LinearRightContinuousPiecewiseConstant<T> {
    /// Construct an interpolator given the rules for extrapolation to
    ...
    pub fn new(
        ...
        left: Extrap,
        right: Extrap,
    ) -> Result<LinearRightContinuousPiecewiseConstant<T>, qm::Error> {
        validate_abscissae(&points)?;
        Ok(LinearRightContinuousPiecewiseConstant {
            left,
            right,
