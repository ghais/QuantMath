...
impl Black76 {
    ...
    pub fn price(&self, df: f64, fwd: f64, k: f64, vol: f64, ttm: f64, q: PutOrCall) -> f64 {
        ...
        return match q {
            PutOrCall::Put => df * (self.cdf(-d_minus) * k - self.cdf(-d_plus) * fwd),
            PutOrCall::Call => df * (self.cdf(d_plus) * fwd - self.cdf(d_minus) * k),
        };
    }
...
impl Bachelier {
    ...
    fn imply_vol_otm(price: f64, df: f64, fwd: f64, k: f64, ttm: f64) -> f64 {
        ...
        let x = if phi < PHI_THRESHOLD {
            let g = 1.0 / (phi - 0.5);
            let xi_numerator = 0.032114372355
        ...
        let x_star_denominator =
            6.0 + q * x * (-12.0 + x * (6.0 * q + x * (-6.0 + q * x * (3.0 + x * x))));
...
fn discounted_intrinsinc(df: f64, fwd: f64, k: f64, q: PutOrCall) -> f64 {
    match q {
        PutOrCall::Call => df * (fwd - k).max(0.0),
        PutOrCall::Put => df * (k - fwd).max(0.0),
    }
}
