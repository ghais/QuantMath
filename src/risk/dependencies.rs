...
mod tests {
    ...
    fn european_dependencies() {
        ...
        let short_expiry = DateTime::new(d + Days::new(70), TimeOfDay::Close);
        let long_expiry = DateTime::new(d + Days::new(210), TimeOfDay::Close);
        ...
        assert_eq!(c.forward_curve_hwm(&equity), Some(d + Days::new(70)));
        assert_eq!(c.vol_surface_hwm(&equity), Some(d + Days::new(70)));
        assert_eq!(c.yield_curve_hwm("OPT"), Some(d + Days::new(72)));
        assert_eq!(c.yield_curve_hwm("LSE"), Some(d + Days::new(70)));

        c.spot(&long_european);

        assert!(c.has_spot(&equity));
        assert_eq!(c.forward_curve_hwm(&equity), Some(d + Days::new(210)));
        assert_eq!(c.vol_surface_hwm(&equity), Some(d + Days::new(210)));
        assert_eq!(c.yield_curve_hwm("OPT"), Some(d + Days::new(212)));
        assert_eq!(c.yield_curve_hwm("LSE"), Some(d + Days::new(210)));
    }
}
